...
**Q1.** La courbe de concentration relie les points qui, en abcsisse,
   ...
   salarié, celui-ci étant égal à `r fmt(saltot)`. Pour les deux
...
On constate que la modalité pour lequel il y a le plus
...
**Q2.** On peut appliquer la formule indiquant que la variance est
   ...
   ^ 2 = 9 - 2.75 ^ 2 = 1.4375$. L'écart-type est la racine carré de
   la variance, soit $\sigma = \sqrt{1.4375} = 1.199$.

**Q3.** Pour calculer les indicateurs de forme, on calcule les écarts par
   rapport à la moyenne pour chaque observation que l'on éleve ensuite
...
z %>% knitr::kable(booktabs = TRUE, escape = FALSE, digits = c(0, 0, 3, 2, 3, 3)) %>%
    kableExtra::kable_styling(position = "center")    
```
  L'indicateur d'asymétrie est obtenue en rapportant le moment centré
  ...
  L'excès de Kurtosis est obtenue en rapportant le moment centré
  ...
  platikurtique, c'est-à-dire plus applatie que la normale.
