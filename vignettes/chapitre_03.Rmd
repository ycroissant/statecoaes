...
- `age` est un caractère quantitatif ; il est discret car il prend
  comme valeur des nombres entiers, mais le nombre de valeurs possible
  est assez élevé, ce qui fait que l'on peut l'assimiler à un
...
1. Le calcul des fréquences cumulées a t'il un sens ? Si oui, réalisez
   le. Que signifie par exemple la seconde fréquence ?
...
**Q2.** Pour calculer les fréquences, on commence par calculer le nombre
   ...
   `r round(fl[3], 2)`$ 
   (`r round(fl[1] * 100)`% des logements ont une seule pièces, `r round(fl[2] * 100)`% en
...
**Q4.** Le graphique le plus adapté pour une variable numérique discrète
   est un graphique en bâton. Pour chaque valeur de la variable, on
...
**Q4.** Les deux graphiques les plus courants pour représenter la
  ...
  - le graphique en barre, pour lequel la hauteur de la barre est
